...
class FallDetectionMLP(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size):
        ...
        # This layer flattens the input from [batch_size, max_len, num_features]
        # to [batch_size, max_len * num_features]
        self.flatten = nn.Flatten()
        
        # A stack of standard "Dense" (Linear) layers
    ...
    def forward(self, x):
        # 1. Flatten the sequence data
        x = self.flatten(x)
        
        # 2. Pass through the standard layers
        x = self.fc1(x)
        x = self.relu1(x)
        x = self.dropout1(x)
...
def train_single_fold(X_train, y_train, X_val, y_val, input_size, config, verbose=False):
    ...
    model = FallDetectionMLP(input_size, config['hidden1'], config['hidden2'], 1)
...
if __name__ == '__main__':
    ...
    config = {
        ...
        'threshold': 0.5,
        'n_folds': 10,
        'test_size': 1/6,  # ~17% held out for final test
    ...
    num_features = 8
    feature_names = ['num_objs', 'avg_x', 'range_x', 'avg_vel', 'std_vel', 'max_vel', 'min_vel', 'avg_snr']
    ...
    if len(filepaths) < 4:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_files, y_train_val)):
            ...
            input_size = X_train_padded.shape[1] * X_train_padded.shape[2]

            result = train_single_fold(
                X_train_padded, fold_y_train,
                X_val_padded, fold_y_val,
                input_size, config, verbose=True
            )
        ...
        input_size = X_trainval_padded.shape[1] * X_trainval_padded.shape[2]
